...
import { 
  ...
  ChevronRight, AlignLeft, Flag, Zap, Bell
} from 'lucide-react';
...
const Inspector: React.FC<InspectorProps> = ({ 
  ...
  useEffect(() => {
    if (task) {
      setFormData({
        ...
        tags: task.tags.join(' '),
        date: task.date || '',
        reminder: (task as { reminder?: string | null }).reminder || '',
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      if (task) {
        ...
        const taskReminder = (task as { reminder?: string | null }).reminder || '';
        const hasChanged = 
          formData.content !== task.content ||
          formData.priority !== task.priority ||
          formData.date !== task.date ||
          formData.reminder !== taskReminder ||
  ...
  const reminderDatePart = useMemo(() => formData.reminder ? formData.reminder.split(' ')[0] : '', [formData.reminder]);
  const reminderTimePart = useMemo(() => {
    if (!formData.reminder) return '';
    const parts = formData.reminder.split(' ');
    return parts.length > 1 ? parts[1] : '';
  }, [formData.reminder]);
  const updateReminderDatePart = (val: string) => {
    const time = reminderTimePart;
    setFormData(prev => ({ ...prev, reminder: val ? (time ? `${val} ${time}` : val) : (time ? ` ${time}` : '') }));
  };
  const updateReminderTimePart = (val: string) => {
    const date = reminderDatePart;
    setFormData(prev => ({ ...prev, reminder: val ? (date ? `${date} ${val}` : ` ${val}`) : date }));
  };
      ...
      <div className="flex-1 overflow-y-auto px-8 space-y-10 pb-20 custom-scrollbar">
        ...
        <section className="grid grid-cols-1 gap-6">
          ...
          <div className="space-y-3">
             ...
             <div className="grid grid-cols-2 gap-3">
                <div className="relative group">
                  <Calendar className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-amber-500 transition-colors" size={14}/>
                  <input 
                    type="date" 
                    value={reminderDatePart}
                    onChange={(e) => updateReminderDatePart(e.target.value)}
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl text-[13px] font-bold outline-none focus:ring-4 focus:ring-amber-500/5 transition-all dark:text-slate-200" 
                  />
                </div>
                <div className="relative group">
                  <Clock className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-amber-500 transition-colors" size={14}/>
                  <input 
                    type="time" 
                    value={reminderTimePart}
                    onChange={(e) => updateReminderTimePart(e.target.value)}
                    className="w-full pl-11 pr-4 py-3.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 rounded-2xl text-[13px] font-bold outline-none focus:ring-4 focus:ring-amber-500/5 transition-all dark:text-slate-200" 
                  />
                </div>
             </div>
             <p className="text-[11px] text-slate-400 dark:text-slate-500 px-1">Browser notification at this time (optional)</p>
          </div>
